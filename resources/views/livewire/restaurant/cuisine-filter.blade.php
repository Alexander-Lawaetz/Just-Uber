<div>
    ...
    <div class="hidden lg:block">
        @forelse ($collection as $item)
            <div
                wire:key="{{ $item->id }}"
                class="{{ $loop->iteration > 8 && !in_array($item->value, $cuisines) && $showMore == false ? ' hidden' : '' }} my-2 bg-light-primary dark:bg-dark-secondary dark:hover:text-dark-important dark:text-light-secondary rounded-lg">
                <label class="relative flex items-end p-2 capitalize">
                    <input
                        ...
                        value="{{ $item->value }}"
                        wire:model="cuisines"
                        {{ in_array($item->value, $cuisines) ? 'checked' : '' }}
                    />
                    {{ $item->description }}
                </label>
            </div>
